...
async function getRates() {
  const stored = await storageGet([RATES_CACHE_KEY, RATES_TIMESTAMP_KEY]);
...
async function runOneClickToggle(tab) {
  ...
  const cfg = await storageGet(['targetCurrency']);
...
async function syncActionPopup() {
  const cfg = await storageGet(['oneClickConvert']);
  const oneClick = !!cfg.oneClickConvert;
  chrome.browserAction.setPopup({ popup: oneClick ? '' : POPUP_PATH });
  updateContextMenu(oneClick);
}

function updateContextMenu(oneClick) {
...
chrome.contextMenus?.onClicked.addListener(async info => {
  ...
  const cfg = await storageGet(['oneClickConvert']);
  await storageSet({ oneClickConvert: !cfg.oneClickConvert });
});
